...
class MiniCourt():
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self, player_boxes, ball_boxes, original_court_key_points):
        # default heights for known players
        player_heights = {
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            closest_player_id_to_ball = min(player_bbox.keys(),
                                            key=lambda x: measure_distance(ball_position,
            ...
            for player_id, bbox in player_bbox.items():
                ...
                closest_key_point_index = get_closest_keypoint_index(foot_position,
                                                                     original_court_key_points, [0, 2, 12, 13])
                ...
                max_player_height_in_pixels = max(bboxes_heights_in_pixels) if bboxes_heights_in_pixels else 1
                ...
                if closest_player_id_to_ball == player_id:
                    closest_key_point_index = get_closest_keypoint_index(ball_position,
                                                                         original_court_key_points, [0, 2, 12, 13])
                    closest_key_point = (original_court_key_points[closest_key_point_index * 2],
                                         original_court_key_points[closest_key_point_index * 2 + 1])
